...
export function LeadsTable({ leads }: { leads: OpportunityLead[] }) {
  ...
  return <div className="overflow-hidden rounded-xl border border-border bg-surface">
    ...
    {!filtered.length ? <div className="p-16 text-center"><CircleAlert className="mx-auto size-5 text-subtle" /><p className="mt-3 text-sm font-medium">Nenhum lead encontrado.</p><p className="mt-1 text-xs text-muted-foreground">Ajuste os filtros ou anexe um arquivo XML.</p></div> : null}

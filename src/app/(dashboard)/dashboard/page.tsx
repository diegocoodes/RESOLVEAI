...
import { BriefcaseBusiness, FileText, FileUp, UsersRound } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const [jobsCount, leadsCount, resumesCount, jobs, leads] = data;

  return <div className="space-y-8"><PageHeader eyebrow="Visão geral" title={`Olá, ${session?.user.name?.split(" ")[0] ?? "Diego"}.`} description="Dados reais do seu workspace, sem métricas fictícias." actions={<><Button asChild variant="secondary" size="sm"><Link href="/prospeccao"><FileUp className="size-3.5" />Anexar XLS</Link></Button><Button asChild size="sm"><Link href="/vagas/nova"><BriefcaseBusiness className="size-3.5" />Cadastrar vaga</Link></Button></>} />
    <section className="grid gap-3 sm:grid-cols-3"><Metric icon={BriefcaseBusiness} label="Vagas" value={jobsCount} /><Metric icon={UsersRound} label="Leads" value={leadsCount} /><Metric icon={FileText} label="Currículos ATS" value={resumesCount} /></section>

...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="grid min-h-screen place-items-center bg-background px-4"><div className="w-full max-w-sm"><div className="mb-8"><span className="grid size-10 place-items-center rounded-lg bg-accent text-sm font-bold text-accent-foreground">DC</span><h1 className="mt-5 text-2xl font-semibold tracking-tight">Entre no diegocodes.com.br</h1><p className="mt-2 text-sm text-muted-foreground">Workspace Diego Codes para oportunidades.</p></div><form action={authenticate} className="space-y-4 rounded-xl border border-border bg-surface p-5"><div><Label htmlFor="email">E-mail</Label><Input id="email" name="email" type="email" autoComplete="email" required /></div><div><Label htmlFor="password">Senha</Label><Input id="password" name="password" type="password" autoComplete="current-password" required /></div>{error ? <p role="alert" className="text-xs text-destructive">E-mail ou senha inválidos.</p> : null}<Button type="submit" className="w-full">Entrar</Button></form><p className="mt-4 text-center text-[11px] leading-5 text-subtle">A conta inicial é criada somente quando o seed é executado com credenciais privadas.</p></div></main>;
}

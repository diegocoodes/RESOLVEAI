...
export default async function NewLeadPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const number = first(query.whatsapp) ?? first(query.phone);
  const initialValues: Partial<LeadInput> = {
    name: contactName, businessName: contactName, niche: first(query.niche)?.slice(0, 100), city: first(query.city)?.slice(0, 100), state: first(query.state)?.slice(0, 2), phone: number?.slice(0, 30), whatsapp: number?.slice(0, 30),

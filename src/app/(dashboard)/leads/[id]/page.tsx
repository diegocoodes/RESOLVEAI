...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!lead) notFound();
  const facts = { name: lead.name, businessName: lead.business, niche: lead.segment, city: lead.location.split(",")[0], state: lead.state ?? null, websiteStatus: lead.websiteStatus === "Sem site" ? "NO_WEBSITE" as const : lead.websiteStatus === "Precisa melhorar" ? "NEEDS_IMPROVEMENT" as const : "UNKNOWN" as const, whatsapp: lead.whatsappValue ?? null, instagram: lead.instagramValue ?? null };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.3fr_0.7fr]">
        <div className="space-y-5">
          ...
          <Card><CardHeader><div><h2 className="text-sm font-semibold">Auditoria do site</h2><p className="mt-1 text-xs text-muted-foreground">Evidência registrada no cadastro</p></div></CardHeader><CardContent>{lead.websiteStatus === "Sem site" ? <div className="flex gap-3 rounded-lg border border-amber-500/20 bg-amber-500/[0.06] p-4"><ShieldAlert className="mt-0.5 size-4 shrink-0 text-amber-400" /><div><p className="text-sm font-medium">Nenhum site próprio informado</p><p className="mt-1 text-xs leading-5 text-muted-foreground">A origem consultada não informou um domínio. Isso não prova que o negócio não possui site em outra fonte.</p></div></div> : <p className="text-sm text-muted-foreground">Consulte a auditoria registrada para este lead antes de usar esta informação.</p>}<p className="mt-4 text-[11px] text-subtle">Quando uma informação não pode ser verificada, ela permanece como não informada.</p></CardContent></Card>

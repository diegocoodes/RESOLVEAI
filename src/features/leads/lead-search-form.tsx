...
export function LeadSearchForm() {
  ...
  return <div className="space-y-5">
    <form onSubmit={search} className="rounded-xl border border-border bg-surface">
      <div className="border-b border-border p-5"><h2 className="text-sm font-semibold">Parâmetros da busca</h2><p className="mt-1 text-xs text-muted-foreground">Busca gratuita em estabelecimentos mapeados pela comunidade OpenStreetMap.</p></div>
      ...
      <div className="flex flex-col justify-between gap-3 border-t border-border p-5 sm:flex-row sm:items-center"><p className="inline-flex items-center gap-2 text-[11px] text-subtle"><ShieldCheck className="size-3.5" />Busca manual e limitada; nenhum dado é salvo automaticamente.</p><Button type="submit" disabled={searching}>{searching ? <Loader2 className="size-4 animate-spin" /> : <Radar className="size-4" />}Buscar oportunidades grátis</Button></div>
    </form>
    {results.length ? <section className="space-y-3" aria-live="polite">
      <div className="flex items-center justify-between gap-4"><h2 className="text-sm font-semibold">Resultados sem site ({results.length})</h2><a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer" className="text-right text-xs font-medium text-muted-foreground hover:text-foreground">© OpenStreetMap contributors</a></div>
      {results.map((result) => <article key={result.externalId} className="rounded-xl border border-border bg-surface p-5">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><h3 className="font-medium">{result.businessName}</h3><p className="mt-1 text-xs text-muted-foreground">{result.formattedAddress ?? "Endereço não informado"}</p>{result.phone ? <p className="mt-1 text-xs text-muted-foreground">{result.phone}</p> : null}</div><div className="flex flex-wrap gap-2"><Button asChild size="sm" variant="secondary"><a href={result.sourceUrl} target="_blank" rel="noreferrer"><ExternalLink className="size-3.5" />Abrir no OpenStreetMap</a></Button><Button asChild size="sm"><Link href="/leads/novo"><Plus className="size-3.5" />Cadastrar após revisar</Link></Button></div></div>
